...
> **クライアント要件変更（2026-09-04）**: ユーザーが持つ学習タスクは1つだけとし、その中の5スロットを設定する。新しいタスクの作成・一覧・削除、`task_id`、`name`、`is_home_task` は不要。現行の `/v1/task-slots` と `/v1/task-slots/options` を継続利用する。

> **クライアント要件変更（2026-09-04）**: タスクスロットの難易度は単一値ではなく、`minimum_difficulty` と `maximum_difficulty` の範囲で指定する。両方 `null` の場合はサーバが推奨レベルを解決する。API接続時にDB・バリデーションへ反映する必要がある。
...
**廃止理由**: 学習タスクを1つに固定したため、このエンドポイントは実装しない。初回提案の確定と通常の編集は `PUT /v1/task-slots/{slot_no}` を使う。以下の契約は廃止前の参考として残す。
**認証是非**: 必須
...
- `tasks` は個別の問題割当、`study_tasks` はホーム画面で切り替える学習スロットを表す。`study_tasks` は `task_no`（スロット番号）順で返す。
- `study_tasks.languages` は最大1件とし、クライアントは言語アイコンを1個だけ表示する。追加枠は表示しない。
- ホームの再生ボタンは選択中の `study_tasks.task_no` を開始対象のスロット番号として利用する。
...
- カレンダーの `tasks` はスロット番号を識別子とし、当日割当時の内容から表示名を生成する。
