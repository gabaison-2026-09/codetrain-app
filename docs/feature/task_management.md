...
現行の `GET /v1/task-slots` などはユーザー単位の5スロットだけを扱い、複数タスクを識別する `task_id` と `name` がない。`ApiTaskRepository` は現行APIを単一の仮想タスクへ変換する暫定実装であり、Composition Rootでは未使用。複数タスクへ本接続する前に、タスク単位の取得・作成・更新・削除エンドポイントを追加する必要がある。
