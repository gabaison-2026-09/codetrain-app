...
- アプリの起動と画面表示は実装済みです。
- 起動時のログイン画面、メール／パスワードとGoogleのモック認証、認証後のホーム表示を実装済みです。認証情報の保存と外部送信は行いません。
...
| --- | --- | --- | --- | --- |
| ログイン | メール／パスワード入力、パスワード表示切り替え、Googleログイン、入力検証、認証後のホーム表示を実装済み | `MockAuthRepository` | Firebase Authenticationを実装するRepositoryへ差し替え。Firebase IDトークンをBearerトークンに使用する想定 | `lib/features/authentication/`、`lib/app/app.dart` |
...
- Repository: `AuthRepository.signInWithEmail()`、`signInWithGoogle()`
