...
- APIレスポンスをアプリ内モデルへ変換するDTOは、トップナビゲーション、学習、タスクの一部に実装済みです。
...
| --- | --- | --- | --- | --- |
| アプリ全体・タブ切り替え | Calendar / Learn / Home / Task / Profile の5タブ、260msのフェード＋横スライド、トップ・ボトムナビゲーションの共通表示を実装済み | 画面内の状態 | API不要 | `lib/features/home/presentation/home_page.dart`、`lib/shared/widgets/` |
...
| Profile | タブとプレースホルダー表示のみ。プロフィール編集・実績表示は未実装 | なし | `GET /v1/me`、`PATCH /v1/me`、`GET /v1/me/stats` | `lib/features/profile/presentation/profile_page.dart` |
...
3. `TopNavigationRepository`、`HomeDashboardRepository`、`LearnRepository`、`TaskRepository` のAPI版実装を作成する。
...
| `PATCH /v1/me` | 表示名・アイコン更新 | Profile画面未実装 |
| `GET /v1/me/stats` | 種別×言語ごとの正答率 | Profile画面未実装 |
