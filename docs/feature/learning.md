...
- 回答後は次の問題へ進み、5問回答するごとに正解数のフィードバックを表示する。
...
スキル／学習項目の選択、問題セットの開始、四択回答、正誤と解説の表示、継続的な問題の進行、直近5問の振り返りを含む5問ごとのフィードバックをモックデータで実装済み。獲得XPは回答直後の結果パネルにのみ表示する。振り返りデータはセッション内のメモリだけで保持し、API通信の具体実装は未接続。
