...
スキル／学習項目の検索・絞り込みと選択、問題セットの開始、ホームタスクの設定に対応した問題の開始、四択回答、正誤と解説の表示、継続的な問題の進行、途中終了、直近5問の振り返りを含む5問ごとのフィードバックをモックデータで実装済み。獲得XPは回答直後の結果パネルにのみ表示する。振り返りデータはセッション内のメモリだけで保持し、途中終了・画面を離れる・アプリを終了すると破棄する。API通信の具体実装は未接続。
