              ...
              builder: (context, constraints) {
                ...
                return SizedBox(
                  ...
                  child: CodeTrainBottomNavigation(
                    extraBottom: systemBottomInset / scale,
                  ),
                );
              },
            ),
...
class CodeTrainBottomNavigation extends StatefulWidget {
  const CodeTrainBottomNavigation({super.key, this.extraBottom = 0});
...
class _CodeTrainBottomNavigationState extends State<CodeTrainBottomNavigation>
  ...
  static const _tabCenters = <double>[118, 302, 483, 668, 851];
  ...
  void _selectTab(Offset localPosition) {
    ...
    for (var index = 0; index < _tabCenters.length; index++) {
      final distance = (x - _tabCenters[index]).abs();
    ...
    if (nearestIndex == _selectedIndex && _controller.isCompleted) {
      return;
    }
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      onTapUp: (details) => _selectTab(details.localPosition),
        ...
        builder: (context, child) {
          return CustomPaint(
            painter: _AnimatedBottomNavigationPainter(
              ...
              selectedX: _tabCenters[_selectedIndex],
              selectedIndex: _selectedIndex,
              selectedLabel: _tabNames[_selectedIndex],
              popProgress: _curve.value,
            ),
            child: child,
          );
        },
...
class _AnimatedBottomNavigationPainter extends _BottomNavigationPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    tray
      ..lineTo(973, 92)
      ..lineTo(973, 324 + extraBottom)
      ..lineTo(0, 324 + extraBottom)
      ..close();
  ...
  bool shouldRepaint(covariant _AnimatedBottomNavigationPainter oldDelegate) {
    return oldDelegate.extraBottom != extraBottom ||
...
class _BottomNavigationPainter extends CustomPainter {
  const _BottomNavigationPainter({required this.extraBottom});
  ...
  bool shouldRepaint(covariant _BottomNavigationPainter oldDelegate) =>
      oldDelegate.extraBottom != extraBottom;
}
